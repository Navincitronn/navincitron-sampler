...
def fetch_public_genius_through_allorigins(target_url: str, *, expect_json: bool) -> Any:
    ...
    try:
        with urlopen(Request(proxy_url, headers=request_headers), timeout=20) as response:
...
def fetch_genius_song_lyrics(song_id: int) -> dict[str, Any]:
    ...
    Preferred source order:
      ...
      3. The same Genius page_data through a public read-only proxy
      4. The same Genius song HTML through a public read-only proxy
      5. LRCLIB text + Genius API referents
      6. lyrics.ovh exact text + Genius API referents
      7. lyrics.ovh catalog suggestion + Genius API referents
    ...
    for source_name, reader in (
        ...
        ("genius_song_page_proxy", lambda: fetch_genius_song_html_proxy(song, song_id)),
    ):
        try:
